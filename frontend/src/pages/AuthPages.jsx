  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--bg-secondary)' }}>
      ...
      <div className="hidden lg:flex lg:w-96 flex-col justify-center px-12" style={{ background: 'var(--bg-primary)' }}>
        ...
        {['Get answers to your toughest questions', 'Help others & build your reputation', 'Track tags & personalize your feed'].map((item) => (
          <div key={item} className="flex items-start gap-3 mb-3">
            <span className="mt-0.5 w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-white text-xs" style={{ background: 'var(--brand)' }}>✓</span>
      ...
      <div className="flex-1 flex items-center justify-center px-4">
        <div className="w-full max-w-sm">
          <h1 className="text-2xl font-bold mb-6 text-center" style={{ color: 'var(--text-primary)' }}>Log in</h1>
          ...
          <p className="text-center text-sm mt-4" style={{ color: 'var(--text-secondary)' }}>
            ...
            <Link to="/register" style={{ color: 'var(--link)' }}>Sign up</Link>
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--bg-secondary)' }}>
      <div className="w-full max-w-md">
        ...
        <div className="card p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" required className="form-input" placeholder="How you'll appear on the site"
                value={form.display_name} onChange={(e) => setForm({ ...form, display_name: e.target.value })} />
            </div>
            <div>
              <label className="form-label">Username</label>
              <input type="text" required className="form-input" placeholder="3–30 chars, letters/numbers/_ -"
        ...
        <p className="text-center text-sm mt-4" style={{ color: 'var(--text-secondary)' }}>
          ...
          <Link to="/login" style={{ color: 'var(--link)' }}>Log in</Link>
        </p>
      </div>
    </div>
  );
